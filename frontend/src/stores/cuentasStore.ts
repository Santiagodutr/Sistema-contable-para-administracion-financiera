  ...
  persist(
    (set, get) => ({
      ...
      registrarMovimiento: (codigo, movimiento) => {
        set((state) => {
          const cuentaExiste = state.cuentas.some(c => c.tipo === codigo)
          ...
          if (cuentaExiste) {
            ...
            // Crear la cuenta global con su primer movimiento
            ...
            const nuevaCuenta: Cuenta = {
              codigo: codigo,
              nombre: nombresCuentas[codigo as keyof typeof nombresCuentas] || 'Otro',
              tipo: codigo as '1' | '2' | '3' | '4' | '5' | '6',
              saldoDebito: movimiento.tipo === 'debito' ? movimiento.monto : 0,
              saldoCredito: movimiento.tipo === 'credito' ? movimiento.monto : 0,
